...
const JualRumah = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const condFieldRes = await fetch(`${endpoint}?mode=get_condition_field`);
                ...
                const condBldRes = await fetch(`${endpoint}?mode=get_condition_building`);
                ...
                const occupancyRes = await fetch(`${endpoint}?mode=get_occupancy_building`);
    ...
    const fetchKota = async (provinsi) => {
        if (!provinsi) return;
        try {
            const res = await fetch(`${endpoint}?mode=get_kota&provinsi=${encodeURIComponent(provinsi)}`);
    ...
    const fetchKecamatan = async (provinsi, kota) => {
        ...
        try {
            const res = await fetch(
                `${endpoint}?mode=get_kecamatan&provinsi=${encodeURIComponent(provinsi)}&kota=${encodeURIComponent(kota)}`
            );
            const data = await res.json();
    ...
    const fetchKelurahan = async (provinsi, kota, kecamatan) => {
        ...
        try {
            const res = await fetch(
                `${endpoint}?mode=get_kelurahan&provinsi=${encodeURIComponent(provinsi)}&kota=${encodeURIComponent(kota)}&kecamatan=${encodeURIComponent(kecamatan)}`
            );
            const data = await res.json();
    ...
    const handleSubmit = async (e) => {
        ...
        if (!selectedProvinsi || !selectedKota || !hargaRaw || !nomorTeleponE164) {
            showToast("Harap lengkapi field wajib: Provinsi, Kota, Harga, dan Nomor Telepon");
            return;
        }
                    ...
                    <div className="flex-1 bg-blue-500 text-white p-6 rounded-xl">
                        ...
                        <form className="space-y-6">
                            <div>
                                ...
                                <input type="text" className="w-full p-2 rounded bg-white text-black" placeholder="Contoh: Green Garden Apartment" />
                            </div>

                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm mb-1">Nama Pemilik</label>
                                    <input type="text" className="w-full p-2 rounded bg-white text-black" placeholder="Contoh: Budi Santoso" />
                                </div>
                            ...
                            <div>
                                ...
                                <input type="text" className="w-full p-2 rounded bg-white text-black" placeholder="Contoh: Jl. Sudirman No. 123, Jakarta Selatan" />
                            </div>

                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                                {[
                                    { label: "Provinsi", list: provinsiList, value: selectedProvinsi, handler: handleProvinsiSelect },
                                    { label: "Kota/Kabupaten", list: kotaList, value: selectedKota, handler: handleKotaSelect },
                                    { label: "Kecamatan", list: kecamatanList, value: selectedKecamatan, handler: handleKecamatanSelect },
                                    { label: "Kelurahan", list: kelurahanList, value: selectedKelurahan, handler: handleKelurahanSelect },
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                                {[
                                    { label: "Kategori Asset", list: assetCategoryList, state: kategoriAset, setter: setKategoriAset },
                                    { label: "Tipe Asset", list: assetTypeList, state: tipeAset, setter: setTipeAset },
                                    { label: "Kondisi Bangunan", list: conditionBuildingList, state: kondisiBangunan, setter: setKondisiBangunan },
                                    { label: "Klasifikasi Jalan", list: classRoadList, state: klasifikasiJalan, setter: setKlasifikasiJalan },
                                ].map(({ label, list, state, setter }) => (
                                    <div key={label} className="relative">
                                        <label className="block text-sm mb-1">{label}</label>
                                        <div
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                                {[
                                    { label: "Jalur Lalu Lintas", list: trafficVolumeList, state: jalurLaluLintas, setter: setJalurLaluLintas },
                                    { label: "Potensi Banjir", list: floodingList, state: potensiBanjir, setter: setPotensiBanjir },
                                    { label: "Tingkat Hunian Bangunan", list: occupancyList, state: tingkatHunian, setter: setTingkatHunian },
                                ].map(({ label, list, state, setter }) => (
                                    <div key={label} className="relative">
                                        <label className="block text-sm mb-1">{label}</label>
                                        <div
                            ...
                            <div className="mt-8 pt-4 border-t border-blue-400">
                                <p className="text-xs italic mb-2">*Cek Ulang Sebelum Klik Verifikasi Data</p>
                                <button
                                    type="button"
